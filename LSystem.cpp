...
void LSystem::processPy(unsigned int n,
	...
	for (int i = 0; i < preBranches.size(); i++)
	{
		...
		std::vector<float> postBranches;

		postBranches.push_back(start[0]);
		postBranches.push_back(start[1]);
		postBranches.push_back(start[2]);
		postBranches.push_back(end[0]);
		postBranches.push_back(end[1]);
		postBranches.push_back(end[2]);

		branches.push_back(postBranches);
	}

	for (int i = 0; i < preFlowers.size(); i++)
	{
		std::vector<float> postFlowers;

		vec3 pos = preFlowers[i].first;
		postFlowers.push_back(pos[0]);
		postFlowers.push_back(pos[1]);
		postFlowers.push_back(pos[2]);

		flowers.push_back(postFlowers);
	}
}
